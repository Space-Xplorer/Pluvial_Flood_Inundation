...
def train_fold(fold_idx, test_sim_idx):
    ...
    model = ConvLSTM(**config.CONVLSTM_CONFIG).to(device)
    ...
    for epoch in range(config.EPOCHS):
        ...
        if test_loss < best_loss:
            ...
            if config.SAVE_BEST_MODEL:
                checkpoint_path = config.CHECKPOINT_DIR / f"convlstm_fold{fold_idx}.pt"
...
def main():
    ...
    print("Training ConvLSTM Baseline Model")
    ...
    # Aggregate results
    print(f"\n{'='*60}")
    print("5-Fold Leave-One-Out CV Results (ConvLSTM)")
    print(f"{'='*60}")
    ...
    # Save results
    results_file = config.RESULTS_DIR / "convlstm_cv_results.pkl"
